...
def _maybe_overtime(obs: Any, task: Any) -> Optional[ConstructionAction]:
    # More conservative overtime than v2 to reduce fatigue drag.
    behind = task.days_behind_schedule >= 2
    important = task.priority in ("critical", "high") or task.is_critical_path
    progressed = task.progress >= 0.35
    enough_workers = obs.workers_available >= max(1, task.required_workers)
    fatigue_ok = obs.overtime_fatigue_level < 0.35
    early_enough = obs.day < (obs.max_days - 6)

    if behind and important and progressed and enough_workers and fatigue_ok and early_enough:
